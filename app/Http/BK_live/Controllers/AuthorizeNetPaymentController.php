...
class AuthorizeNetPaymentController extends Controller
{
   ...
   // private $amount=10;
    ...
    public function refundTransaction($transactionID='',$cardNumber='',$expireDate='',$amount=0,$refTransId='')
    {
        ...
        echo $amount;
        echo "<br>";
        echo "REF".$refTransId; die();*/

        //$cardNumberLength=strlen($cardNumber);
        //$startLength=$cardNumberLength-4;
       // $cardNumber=substr($cardNumber, $startLength);
        //echo $cardNumber; die();
    ...
    public function captureCardPayment($refId=0,$cardNumber=0,$expireDate='',$amount=0,$invoice_id=0)
    {
        ...
        // Create a transaction
          ...
          $response = $controller->executeWithApiResponse(\net\authorize\api\constants\ANetEnvironment::PRODUCTION);  
           ...
           if($this->sdc->storeID()==121){
         	//8vxkeM6RL7e62an2PnE8WLqW2t4a8Z2J665VAY7NE8acAWC876PByKemdeMzHzRf
           ...
           if(empty($response)){
           		$arrayReturn=[
                        'status'=>4,
                        'message'=>'Please contact with system admin. There is some issue with your configuration.'
                ];

                return $arrayReturn; die();
           }

          //print_r($response); die();
          	$tabAct=new Activity();
	        $tabAct->activity_type="Authorize Card Payment";
	        $tabAct->activity=json_encode($response);
	        $tabAct->store_id=$this->sdc->storeID();
	        $tabAct->created_by=$this->sdc->UserID();
	        $tabAct->save();
          ...
          if ($response != null) 
          {
                ...
                $tabAct=new Activity();
                $tabAct->activity_type="Authorize Card Payment";
                $tabAct->activity=json_encode($tresponse);
                $tabAct->store_id=$this->sdc->storeID();
                $tabAct->created_by=$this->sdc->UserID();
                $tabAct->save();
